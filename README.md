...
현재 계획 (2026-09-22): RGB 영상으로부터 LingBot-Map으로 공간을 복원하고, 결과 포인트클라우드에 직접 **3D instance segmentation**을 적용한다. 분리된 실제 관측 형상을 객체별 메시로 만들고, 색상·텍스처가 포함된 **GLB 파일**로 저장해 웹에서 확인한다.

```mermaid
flowchart TD
    A["RGB 영상 입력"] --> B["프레임 추출·전처리"]
    B --> C["LingBot-Map: 점군·깊이·카메라 정보"]
    C --> D["점군 전처리: 스케일·밀도·좌표계 정리"]
    D --> E["3D instance segmentation: 객체별 점 마스크"]
    E --> F["원본 점에 객체 ID 연결·분할 결과 정리"]
    F --> G["객체별 메시 생성"]
    G --> H["색상·텍스처 적용"]
    H --> I["객체별 GLB + 장면 메타데이터"]
    I --> J["웹 3D 뷰어"]
```

| 단계 | 처리 계획 | 주요 산출물 |
| --- | --- | --- |
| 영상 전처리 | FFmpeg 또는 OpenCV로 프레임을 추출하고 원본 프레임 번호·시각을 기록한다. | RGB 프레임, 영상 대응 정보 |
| 공간 복원 | LingBot-Map으로 깊이와 카메라 위치·자세를 추정해 공통 좌표계의 점군을 만든다. 후속 표면 복원·텍스처링을 위해 프레임별 RGB·깊이·신뢰도·카메라 정보도 보존한다. | XYZRGB 점군, 카메라·깊이·실행 기록 |
| 점군 전처리 | 선택할 분할 모델의 입력 조건에 맞춰 유효점 검사, 노이즈 정리, voxel 다운샘플링, 좌표계·스케일 처리를 수행한다. 원본 점과 계산용 대표점의 대응 및 좌표 변환을 저장한다. | 계산용 점군, 원본 대응 인덱스, 변환 정보 |
| 3D 인스턴스 분할 | XYZRGB 점군을 직접 입력받는 모델로 개별 객체의 3D 점 마스크를 예측한다. 사용할 모델은 아직 확정하지 않았다. | 객체별 점 마스크, 모델이 제공하는 분할 점수 |
| 객체 정보 정리 | 마스크 중첩과 작은 조각을 검토하고 원본 점에 객체 ID를 연결한다. 미할당·불확실한 점은 별도로 유지한다. 클래스는 모델이 제공하면 연결하고, 이름을 알 수 없는 객체는 미확정으로 저장한다. | 객체별 원본 점 집합, instance_id, 클래스·관측 근거 |
| 메시·텍스처 생성 | Open3D 기반으로 객체 표면을 복원한다. TSDF를 사용할 경우 분할된 점과 대응하는 깊이 관측을 연결한다. RGB 색상 또는 OpenMVS 텍스처를 적용하며 객체 사이를 잇는 면을 제거한다. | 객체별 삼각형 메시, 색상·텍스처 |
| 저장·웹 표시 | 객체별 GLB와 장면 배치 정보를 저장한다. 웹에서 영상 업로드, 처리 상태 확인, 객체 선택·숨김·정보 조회·다운로드를 제공한다. | 객체별 GLB, scene.json, 웹 뷰어 |

현재 주 흐름에서는 프레임별 2D segmentation, 2D 마스크의 3D 투영·투표, 마스크 기반 객체 병합을 필수 단계에서 제외한다. 슈퍼포인트가 필요한지는 선택한 3D 분할 모델의 내부 처리 방식에 따른다. RGB 프레임과 카메라 정보는 표면 복원·텍스처링·관측 근거 확인을 위해 계속 보존한다.

인스턴스 분할은 같은 종류의 물체도 각각 구분하는 단계다. 예를 들어 두 의자에는 서로 다른 객체 ID를 부여한다. 클래스 이름 부여는 별도로 확인해야 하며, 모델이 이름을 제공하지 않으면 후속 분류로 보완한다. 벽·바닥·천장도 출력 대상이므로 선택 모델의 구조 영역 처리 여부를 확인하고, 필요한 경우 해당 영역의 의미 분류·평면 분리를 추가한다.

최종 산출물은 `objects/object_0001.glb`와 같은 객체별 파일, 객체 ID·클래스·파일 경로·위치·방향·크기·좌표계·단위·근거 프레임을 연결하는 `scene.json`, 재처리용 원본 점군이다. 클래스가 확인되면 `chair_001.glb`처럼 이름을 붙일 수 있다. 좌표는 공통 장면 좌표계를 유지하거나, 객체 좌표를 사용할 경우 장면 변환을 명시한다.

구현할 때는 LingBot-Map의 좌표 단위를 확인하고 모델이 미터 단위를 요구하면 실측 기준으로 보정한다. 전처리 변환은 점군·깊이·카메라 사이에서 일관되게 적용한다. 분할 결과는 원본 점으로 되돌릴 때 객체 경계와 미할당 영역을 확인한다. 촬영되지 않은 면과 객체 경계의 구멍은 품질 검토 대상으로 기록한다.

현재 확정된 것은 **점군에 직접 3D instance segmentation을 적용하는 설계 방향**이다. 모델 선정과 전체 자동 처리 연결은 후속 구현 과제다. 기존 Utonia semantic 분류, MV3DIS 다중 시점 분할, FIRE3D 복원은 비교 실험 기록으로 유지한다.

향후에는 대표 프레임의 VLM 분석을 활용한 객체 이름·설명 보완, 촬영 회차 간 객체 변화 비교, 360도 영상 지원으로 확장한다. 360도 영상은 여러 방향의 일반 시야각 영상으로 변환하는 방식을 검토한다.
...
이번 결과는 3D 지도에 의미 라벨을 부여하는 기초 비교 자료로 사용할 수 있다. 현재 계획에서는 XYZRGB 점군을 직접 처리하는 3D instance segmentation 모델을 선정해 개별 객체 마스크와 ID를 얻는다. 모니터·키보드·사람 등의 클래스 이름은 선택 모델의 지원 범위를 확인하고 필요한 경우 후속 분류로 보완한다. 다음 실험에서는 같은 영역에 정답 라벨을 만들고 객체별 점 IoU, 과분할·과병합, 작은 물체의 경계 품질을 평가한다. 클래스 정확도는 객체 분리 품질과 구분해 기록하고, 촬영 회차 간 추적은 별도의 객체 대응 단계로 다룬다.
...
- 이 GLB에는 형상과 실제 RGB 텍스처를 저장했다. 현재 계획한 3D instance segmentation의 객체 ID와 확인된 클래스 정보를 객체별 메시로 연결하는 작업은 후속 단계다.
